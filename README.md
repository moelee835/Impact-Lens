...
- Python/FastAPI의 일반 호출은 provider 지원 범위에서 나타나지만, route handler(`@app.get()` 등으로
  선언)와 `Depends()` 의존성은 둘 다 정적 Call Hierarchy에서 보이지 않습니다 — 둘 다 실제로는
  FastAPI가 요청 처리 시점에 직접 호출하지만, 그 호출이 분석 대상 코드 안의 call expression이 아니라
  프레임워크 내부(router dispatch·dependency resolver)에서 일어나기 때문입니다. 실제 FastAPI로
  측정해 둘 다 같은 신호(`provider_null_incoming_calls`)로 확인된 사실입니다.
...
- `limitationDetails`의 `provider_null_incoming_calls`: 특정 provider 하나의 한계가 아니라 **모든
  ...
  handler와 `Depends()` 대상 둘 다 실제로는 FastAPI가 호출합니다(전자는 router dispatch, 후자는
  dependency resolver — `Depends()`가 실제로 함수를 호출해 반환값을 주입하는 것은 dependency
  injection의 정의이고, 실제 FastAPI 코드에 함수 실행 흔적을 남기고 `TestClient`로 요청을 보내
  직접 확인했습니다), 다만 그 호출이 분석 대상 코드 안의 call expression이 아닐 뿐입니다. 둘 다 이
  신호로 나타난다는 것 자체는 실제 FastAPI 코드로 측정해 확인한 사실입니다(추정 아님).
