...
pnpm run compile
git diff --check
pnpm exec vsce package --out /tmp/impact-lens-0.6.2.vsix
```
...
```sh
unzip -l /tmp/impact-lens-0.6.2.vsix
```

artifact checksum을 생성한다.

```sh
shasum -a 256 /tmp/impact-lens-0.6.2.vsix
```
...
```sh
code --install-extension /tmp/impact-lens-0.6.2.vsix --force
```
