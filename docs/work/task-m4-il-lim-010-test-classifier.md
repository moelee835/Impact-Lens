...
**(C) commander가 명시적으로 요구했던 세 번째 목록 — 처음엔 빠졌다가 지적받고 추가함**: (A)/(B)는
"오늘 갈라지는 행"만 다룬다. **오늘 두 host가 이미 일치하던 행 중에서도 규칙 재정의로 답이 바뀌는
행이 있고, 이건 (A)/(B) 어디에도 안 잡힌다.** 이 세션의 2808행 corpus로 직접 집계: **일치하던
2582행 중 322행이 새 규칙에서 답이 바뀐다** — commander가 독립적으로 보고한 322와 정확히 일치.
대표 예시(전부 `[실행]` 확인, 오늘 두 host `true` → 새 규칙 `false`): `foo_test.ts`, `test_foo.ts`,
`FooTest.ts`, `foo.test.py`, `foo_test.java`, `Test_foo.rb`. **이 322건 각각은 정확성 문제가
아니다** — Jest는 `foo_test.ts`를, pytest는 `foo.test.py`를, Surefire는 `foo_test.java`를 기본
설정으로 수집하지 않으므로 새 답이 옳다. **문제는 처음에 이 목록을 사용자 가치/PR 설명에서
누락했다는 것**이었다 — story의 "예고 없는 변화 금지" 의도는 정확성이 아니라 공개를 요구한다. PR
#91 본문과 이 문서 모두 이제 이 322건을 명시한다. 실사용 영향은 (B)/(C)를 합쳐 판단해야 한다 —
TS 프로젝트에서 `foo_test.ts` 관례를 쓰던 사용자는 그 파일이 관련 테스트 목록에서 사라지는 걸
본다.
...
그 언어 확장을 실제로 설치했을 때만 도달한다. 그러므로 아래 정정된 숫자도 상한이지 정확한 도달
보장 수가 아니다.

**2026-09-09 정정(같은 날, 후속) — R1을 `.d.ts`에 적용한 게 틀렸다. 140이 아니라 182다.** 위
문단은 R1(선언 전용 확장자는 depth 0에서만 나타나 단락된다)을 `.pyi`와 `.d.ts` 둘 다에 적용했다.
reviewer가 실제 tsserver로 반증했다: **ambient-context 위반**(구현부를 담은, `tsc`가 TS1183으로
거부하는) `.d.ts` 파일을 만들어 실제로 call hierarchy에 넣어 보니 **depth 1 노드로 들어온다.**
`.d.ts`의 "선언 전용"은 타입체커 진단이지 파서 차단이 아니어서, tsserver는 그런 파일도 그냥
통과시킨다. `.pyi`(reviewer가 실제 bundled pyright로 typeshed root를 확인한 건과 별개 근거) 제외는
여전히 유효하지만 **`.d.ts` 제외는 무효다.**

이 세션이 R1을 `.pyi`에만 적용하도록 좁혀 독립 재현했다: **도달 가능 분모는 140이 아니라 182**
(그 차이 42건 전부 `.d.ts` shape). 182건 전부에 이미 커밋된 최종 규칙을 다시 적용해 재검산:
**ext 쪽 178건 변경 + cli 쪽 4건 변경 = 182, 미해결 0건** — checksum은 여전히 정확히 맞는다.
**결론은 안 바뀐다**: `.d.ts` 배제 자체는 Jest 기본 `testMatch`가 `.d.ts`를 안 모은다는, reachability와
무관한 근거로 여전히 옳다(위 규칙 테이블 주석도 이 근거만 남기도록 정정했다 — "선언 전용이라
실행 코드가 없다"는 첫 버전의 이유는 이번 반증으로 틀렸음이 드러나 지웠다). **바뀐 건 도달 가능
분모의 숫자와, `.d.ts`를 "CLI 버그"/"도달 불가"라고 부른 근거뿐이다** — 코드는 이미 옳았다.

**주석-코드 불일치 하나 추가 정정**: `dot-suffix` 규칙의 최초 주석("a.test.d.ts도 매치한다")이
바로 그 규칙의 코드(`.d.ts`를 명시적으로 배제)와 반대로 적혀 있었다 — commander가 실행으로 발견,
바로 아래 코드와 일치하도록 정정했다(`cli/src/shared/testFileClassifier.ts`).
