...
6개(회귀 없음), **오탐 0개**, **recall 대상(A) 안에서는 위음성 0개(6/6), 그러나 dispatch의
`Depends()` 참조 14개 전체 기준으로는 6/14(43%)만 찾는다** — 나머지 8개(B+C)는 여전히
위음성이다. 기각이 오탐을 없앤 것이지 recall을 완성한 게 아니다. **네 번째 형태(C,
