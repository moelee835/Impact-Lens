...
신규 지침이므로 patch가 아니라 minor다.

**2026-09-10 정정(reviewer 지적)**: 위 diff에 담긴 내용은 순수 추가이지만, `+139 -0`이라는
숫자 자체는 부정확했다 — 그 diff는 B-3(이 문서 자신의 버전 반영 커밋)이 `cli-contract.md`의
예시 JSON 안 버전 문자열을 `0.8.0`→`0.9.0`으로 바꾼 것까지 같은 범위(`v0.8.0..HEAD`)에 걸려
있어, 실제로는 **삭제 3줄**이 있다(전부 예시 응답의 `"version": "0.8.0"` 텍스트가
`"version": "0.9.0"`으로 바뀐 흔적이지, 내용이 지워진 게 아니다). **판정(minor)은 안 바뀐다** -
agent가 따라야 할 실제 판정 규칙(candidate-caller 어휘, augmentation code 5종, Go/C confirmed
각주)은 여전히 전부 새로 추가된 것뿐이고, 그 판정 규칙과 무관한 버전 문자열 치환 3줄을 "필드
제거·재정의"로 셀 이유가 없다 - 다만 "순수 추가"라는 표현 자체는 부정확했으므로 정정한다.
