...
## 패턴 — 주석이 주장하는 보장과 코드가 실제로 하는 일이 어긋난 사례 7건
...
>
> **2026-09-10 재재재갱신(M3 Java stage 3 사전 작업, `docs/work/
> task-m3-java-project-import-readiness.md`)**: 6건에서 **7건**으로 늘었다. 아래 7번이 추가됐다 —
> 앞의 여섯과 다른 세 번째 하위 종류다(1~3·5·6번은 "실제보다 넓게" 말했고, 4·6번은 "엉뚱한 곳을
> 가리켰다"; 7번은 "실제보다 좁게" 말했다).

**주석이 주장하는 보장과 코드가 실제로 하는 일이 어긋난 사례가 이 마일스톤에서 7건 나왔다.**
1~6번은 읽기가 아니라 실행·대조로 발견됐다 — **7번은 다르다**, 읽어서 확인됐을 뿐 실행 검증은
아니다(아래 7번 항목, `읽어서 확인 ≠ 실행으로 확인` 참고). 다음 사람이 이 저장소의 주석을 근거로
삼기 전에 알아야 할 사실이라 여기 남긴다(이 lane은 이 주석들을 고치지 않는다 — 기록만 한다):
...
7. **`docs.limitations` "executable 없을 때만 보임" 주석**(M3 Java stage 3 사전 작업,
   `cli/src/providers/preset.ts:248-252`): `ProviderPreset.docs`의 doc-comment가 "Shown to the
   user when the executable is missing"라고 적는다. 그런데 실제 소비 지점(`cli/src/doctor/
   index.ts:174-181`)은 `...(preset.docs?.limitations === undefined ? {} : { limitations:
   preset.docs.limitations })` — 필드 존재 여부만 조건이고, executable 탐색 결과와는 무관하게
   doctor가 이 preset을 보고할 때마다 **항상** 포함된다. 이번 lane이 코드를 직접 읽어 확인.

**일곱 번이면 우연이 아니다.** 이 저장소의 주석·문서는 코드가 보장하기를 **의도한 것**을 적는
경향이 있고, 일곱 번 다 그 보장이 실제로는 코드와 어긋났다.
...
**7번은 앞의 여섯과 다른, 세 번째 하위 종류다.** 1~3·5·6번은 주석·문서가 **실제보다 넓게**
말했다(없는 보장을 약속했거나, 약속이 근거의 범위를 넘었거나, 참인 주장의 근거를 엉뚱한 곳으로
가리켰다). 7번은 반대다 — **실제보다 좁게** 말했다: 코드는 항상 보여주는데 주석은 "없을 때만
보인다"고 적었다. 이번엔 결과적으로 유리한 방향이었다(어차피 늘 보이므로 사용자가 놓치는 정보는
없다) — 하지만 **다음 사람이 이 주석을 근거로 "executable이 있으면 이 필드는 안 보이겠구나"라고
설계하면 틀린다.** 좁게 말하는 주석은 넓게 말하는 주석과 똑같이 신뢰할 수 없다는 뜻으로, 이 저장소가
전에 만난 적 없는 실패 방향이다. **그리고 7번은 발견 경로도 다르다** — 1~6번은 실행·대조로 잡혔지만
7번은 코드를 직접 읽어서 확인됐다(실행 검증은 아니다) — 이 저장소가 `읽어서 확인 ≠ 실행으로 확인`을
구분해 온 것과 같은 이유로, 이 항목의 확실성 등급도 앞의 여섯과 같지 않다고 명시해 둔다.
...
# 2026-09-10 — Gate 8 판정과 **M4 종료: 8개 gate 전부 닫힘**

**Gate 8 — 닫힘.** `docs/development-management/user-tests/m4-user-test-spec.md` 작성·검토 완료
(PR #108). **실행하지 않았다.**

gate 문구가 **"사용자 결과 또는 보류 사유"**를 허용한다. 그래서 이 gate는 **명세가 존재하고 검토를
거쳤으며, 보류 사유와 그것이 rollout 결정에 갖는 연결이 명세 안에 적혀 있을 때** 닫힌다. 보류 사유는
**naive 참여자 미모집**이고, 연결은 **augmentation 기본값 off 유지**다.

**명세가 스스로에 대해 적는 것**(이게 이 gate를 정직하게 닫는 조건이다):
- 유지보수자가 지금 혼자 수행 가능한 **Part A**와 naive 참여자가 있어야만 성립하는 **Part B**를 갈랐다.
- **Part A를 전부 통과해도 그것은 사용자 검증이 아니다** — 제품을 만든 사람이 자기 제품을 돌린 것이고,
  gate 8이 실제로 묻는 "사람이 그 값을 그렇게 읽는가"는 **여전히 미측정으로 남는다.**
- **이 문서는 명세일 뿐이며 존재만으로 사용자 검증을 통과한 것으로 표시하지 않는다**(M2 명세 계승).

**Part A의 모든 명령은 실제로 돈다** — 작성자가 실제 빌드된 CLI로 전부 돌려 자기 초안의 오류 넷을
잡았고(import 누락, `setTimeout` 위치, 줄 번호, alias 형태), reviewer가 **깨끗한 worktree에서 새로
빌드하고 `gopls`를 새로 설치해 독립 재현**했다. **명령이 안 도는 "수행 가능한 가이드"는 없는 것보다
나쁘다** — 사용자가 자기 환경을 의심하게 만든다.

**Part B의 blind 규율**은 질문 문구 단위로 검증됐고, Go/C 과업(B5)은 **3단계 공개 순서를 바꾸면 그
세션은 무효**라는 규칙이 별도 절에 있어 이중으로 강제된다. B5는 이 명세에서 가장 값진 과업이다 —
**PR #106이 추가한 경계 문구가 실제로 오해를 푸는지를 재는 유일한 방법**이고, 문서를 고쳤다는 것과 그
문서가 통한다는 것은 다르며 후자는 아무도 잰 적이 없다.

## 이 마일스톤을 닫으며 인용을 열어 본 결과

**닫는 작업 자체에서 잘못된 인용이 세 건 나왔고, 셋 다 merge 전에 잡혔다** — 잡힌 이유는 하나뿐이다:
**누군가 인용된 파일을 실제로 열었다.**
...
- 같은 문단: sub-dependency fixture를 `nested_dependency_config/db/consumer.py`라고 적었는데 **그런
  경로는 존재하지 않는다** — 평평한 세 파일(`nested_dependency_config.py`/`nested_dependency_db.py`/
  `nested_dependency_consumer.py`)의 이름을 이어 붙여 만든 **가짜 중첩 경로**였다. 앞의 둘이 "엉뚱한
  곳을 가리킨 것"이라면 이건 **가리킨 곳이 아예 없는 것**이라 한 단계 더 나쁘다.

**운영 규칙**: 이 저장소에서 **인용은 열어 보기 전까지 근거가 아니다.** 위 "주석이 주장하는 보장과
코드가 실제로 하는 일이 어긋난 사례" 절이 기록한 여섯 건 중 둘이 이미 "가리키는 인용" 유형이었고,
이번 닫는 작업에서 셋이 더 나왔다 — **빈도 자체가 신호다.** 파일명·경로·줄 번호를 인용할 때는
`grep`/`ls`로 실재를 확인한 뒤 적는다.

## M4 최종 — 8개 중 8개 닫힘

| gate | 판정 |
| --- | --- |
| 1. `IL-LIM-001`·`002`·`010` 수용 기준 | 닫힘 — 명시된 잔여를 안고(통과 12 / 부분 1 / 세 번째 상태 1) |
| 2. JSON·UI 구분 | 닫힘 — 명시된 미검증 범위를 안고(extension-host harness 부재) |
| 3. alias·sub-dependency·cross-file fixture | 닫힘 — 대체 gate로. Spring 형태는 이월 |
| 4. 임의 승격 금지 | 닫힘 — 수용된 잔여 1건을 안고 |
| 5. path convention이 가짜 edge·test passed를 만들지 않음 | 닫힘 — **부재에 의한 통과**, 3단계 도래 시 재판정 |
| 6. augmentation off = 안전한 rollback | 닫힘 |
| 7. false-positive·latency budget | 닫힘 — 명시된 잔여 7건을 안고 |
| 8. 사용자 테스트 명세 | 닫힘 — 명세 작성·검토 완료, **실행하지 않음**(보류 사유가 rollout에 연결) |

**여덟 중 여섯이 "잔여를 안고" 닫혔다. 그게 이 마일스톤의 형태다** — 이 저장소의 기준은 "잔여 0건이어야
닫힌다"가 아니라 **"잔여가 정확히 이름 붙어 있으면 닫힌다"**이고, 이름 붙지 않은 잔여가 하나라도 있으면
그 gate는 닫히지 않았다.

## 이 종료가 뜻하지 않는 것

1. **기능을 기본값으로 켜도 된다는 뜻이 아니다.** augmentation은 두 host 모두 **기본값 `false`**로
   남는다. 근거: 실측 recall 약 57%, 명시된 잔여, **실사용자 검증 0건**. 사용자가 2026-09-10에 이
   상태로 출하하기로 결정했다.
2. **사용자 검증이 끝났다는 뜻이 아니다.** gate 8은 **명세**를 요구하고 명세는 완성됐다. **실행은
   여전히 미수행**이고, 그것이 gate 문구가 "보류 사유"를 허용하는 이유다.
3. **세 story가 닫혔다는 뜻이 아니다.** `IL-LIM-001`·`IL-LIM-002`·`IL-LIM-010` 전부 그대로
   `Backlog`이며, 셋 다 이 마일스톤 밖의 후속 단계를 갖는다. **수용 기준이 통과하는 것과 story가
   닫히는 것은 다른 측정이다.**
4. **`data.edges`가 모든 언어에서 정확하다는 뜻이 아니다.** `gopls`·`clangd`가 호출 없는 순수 참조를
   `relation: direct`로 보고한다 — 어느 gate도 이것을 묻지 않았고, 그래서 gate 통과가 이 층의 정확성을
   보증하지 않는다. 문서화 완료(PR #106), 수정은 별도 이슈.

## 다음

릴리스(버전·CHANGELOG·태그)는 **"M4 완료"와 별개의 후속 작업**이다 — M2가 같은 구분을 썼다
(`task-m2-release-0-8-0.md`). 사용자 결정으로 **`v0.9.0`**으로 발행한다.

---

# 2026-09-11 — 사용자가 그래프 렌더링을 확인했다. **gate 2의 시각 절반은 여전히 열려 있다.**

**사실**: v0.9.1 설치 후 사용자가 **"그래프 보임"**을 확인했다. `v0.7.0`·`v0.8.0`·`v0.9.0` 세 릴리스
동안 한 번도 그려지지 않던 Call Graph 패널이 **실제 사용자 화면에서 그려진다.**

**이 확인 전까지 우리가 증명한 것은 "생성된 스크립트가 파싱된다"까지였다.** 발행본 아티팩트 안의
`graphPanel.js`를 실제로 불러 `getHtml()`을 호출하고 파싱까지 확인했지만, **그것은 필요조건이지
충분조건이 아니다.** B-4(발행 경로 검증)도 CLI 경로만 훑고 VS Code를 띄우지 않는다.

**그래서 이 결함은 사용자 보고로 발견됐고 사용자 확인으로 닫혔다.** 우리 자동 검증은 **발견도 닫힘도
하지 못했다** — 그 모듈을 **한 번도 실행한 적이 없었기 때문**이다(`vscode` import 때문에 plain
`node --test`로 부를 수 없어, 모든 검사가 소스 텍스트 정규식이었다).

## 이 확인이 닫지 않는 것 — 혼동하면 안 된다

**"그래프가 보인다"와 "candidate edge가 시각적으로 구별되고 라벨이 겹치지 않는다"는 다른 주장이다.**

gate 2는 **JSON 절반은 실행으로 검증하고, 시각 절반은 명시된 미검증 범위를 안고** 닫혔다 — 이
저장소에 extension-host harness가 없어 **실제 webview 렌더, marker의 시각적 구별, 라벨 겹침을 한 번도
확인하지 못했다**는 것이 그 gate 자신의 기록이다.

**사용자가 확인한 것은 "패널이 내용을 그린다"까지다.** marker가 실제로 구별되는지, 라벨이 겹치지
않는지는 **묻지 않았고 확인되지 않았다.** 그 검증은 `user-tests/m4-user-test-spec.md`가 정의한 채로
**아직 아무도 수행하지 않았다**(gate 8이 "명세 작성·검토 완료, 실행하지 않음"으로 닫힌 그대로다).

**다음 사람이 "사용자가 확인했다"를 gate 2의 시각 절반까지 덮는 것으로 읽지 않도록 여기 적는다.**
