...
> 4번은 **현재 통과이지만, 통과의 근거가 안전장치가 아니라 기능의 부재다.** 이 구분을 안 적으면 다음
> 사람이 "검증된 항목"으로 읽는다.
>
> 지금 이 저장소에는 **`passed`/`failed` 계열 상태가 아예 없다** — Extension의 `src/types.ts`가
> `TestFreshness = 'notRun' | 'outdated'` 둘만 두고, Agent CLI의 `cli/src/types.ts`에는 테스트 실행
> 어휘가 **한 개도 없다**(둘 다 직접 확인). 즉 "실행하지 않은 테스트를 성공으로 표시"하는 것이 **구조적으로
> 불가능**하다. 위반을 막는 장치가 검증된 것이 아니라 **위반할 기능 자체가 없는 것**이다.
>
> **따라서 이 기준을 위해 새 상태나 새 모델을 만들지 않는다** — 그건 지금 없는 위험을 스스로 만든 다음
> 그것을 막는 장치를 만드는 일이다.
>
> **그리고 이 판정은 3단계(실행 결과 import)가 도래하는 순간 재실행되어야 한다.** 3단계는 정의상
> stale/partial/failed/passed를 구분하는 어휘를 도입하므로, 도입되는 그 시점에 이 기준의 근거가
> "부재"에서 "장치"로 바뀌어야 하고 그 장치는 실제로 검증돼야 한다. **이 gate가 이미 닫혔다는 이유로
> 3단계에서 이 항목을 건너뛰지 않는다.**
