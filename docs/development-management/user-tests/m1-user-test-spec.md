...
네 가지를, **실제 사람이 provider 내부 지식 없이** 달성하는지를 본다. 자동 검사(W1~W3-A)는 이미 이 네
가지의 *기계 쪽 절반* — 정확한 값이 반환되는가 — 을 증명했다. 이 테스트는 나머지 절반을 본다.
...
| Project (T1·T2·T4용) | 참여자 본인의 실제 TS 또는 JS 저장소 1개 + 공통 sample 저장소 1개 |
...
| S3 | 손상된 `.impact-lens/provider.json` | 진행자가 세션 시작 **전에** 참여자 워크스페이스에 `.impact-lens/provider.json`을 미리 만들어 두되, 허용되지 않은 필드를 하나 포함시켜 둔다(예: `{"presetId": "bundled-typescript", "typo": true}` — `ALLOWED_FIELDS`는 `presetId`/`command`/`args`/`languageId`/`initializationOptions`/`settings`뿐이다, `cli/src/providers/projectConfig.ts:17`). 참여자에게는 무엇을 손상시켰는지 알려주지 않는다 |
| S4 | `complete: true` 빈 결과 해석용 준비된 응답 | §4에서 고정한 sample 함수를 대상으로 진행자가 `analyze`를 **실제로** 실행해 얻은, 그대로의 JSON. 손으로 쓰거나 편집하지 않는다 — R3가 요구하는 "real" 응답이다 |

준비 원칙은 M0와 같다. 깨진 상태는 세션 범위로만 만들고, 시스템·source·사용자 홈을 손상시키지 않는다.
S3는 workspace 안의 파일 하나를 추가하는 것뿐이므로 시스템 상태에 영향이 없다.

## 6. 과업

과업은 outcome으로 제시한다. 진행자는 내부 provider command, args, `languageId`, preset id, 오류 code
목록을 **사전에 설명하지 않는다.** 참여자가 먼저 묻는 경우 "문서와 도구 출력에서 찾아보라"고만 안내한다.
T4는 별도 규칙이 있다 — 아래 참고.
...
1. **자유 서술 먼저.** S4에서 준비한 JSON 응답(또는 host UI에 그대로 렌더링된 화면)을 참여자에게 보여주고
   정확히 이렇게만 묻는다: "**이 결과를 보고, 이 함수에 대해 다음에 무엇을 하시겠습니까?**" 선택지를
   주지 않는다. `complete`, `caveat`, `안전` 같은 단어를 진행자가 먼저 꺼내지 않는다. "이 결과가
   안전해 보이나요?" 같은 질문은 답을 심는 질문이므로 **금지**한다(§7 유도 편향 규칙).
...
| T2 | `.impact-lens/provider.json`이 원인임을 스스로 지목하고, 오류 메시지 또는 `doctor` 출력만으로 파일을 고쳐 복구 | 오류 메시지를 읽고 직접 수정 / `doctor bundled-typescript` 실행 후 `project-config` check를 읽고 수정, 무엇이든 | 20분 초과, 또는 진행자에게 무엇이 문제인지 직접 질문해 답을 들음(그 순간 "진행자가 유출"로 기록하고 중단) |
...
- T4: 자유 서술 원문(§11 규칙 안에서), 정적 범위 한계를 자발적으로 언급했는가, 언급했다면 자유 서술
  단계였는가 후속 질문 단계였는가, 최종적으로 "지운다/안 쓴다"고 판단했는가
...
- T4의 자유 서술 단계에서, 함수를 지우거나 안 쓴다고 판단한 참여자 중 정적 범위의 한계를 스스로 언급하지
  **않은** 비율이 낮을수록 좋다 — 구체적 임계값은 baseline 이후 정한다. 0명이 언급하면 그 자체가
  release를 보류할 근거다(§10 보류 기준).
...
과업이 검증하는 것이 바로 그 문구가 실제 사람에게 통하는지이기 때문이다.
...
- 수집 허용: redacted 오류 JSON의 `error.code`, `error.details.stage`, `runtime.runner.source`,
  ...
  정보, 시각과 판정, T4의 발화 요약(동의 범위 안에서 원문 인용 포함).
...
- [ ] 깨진 상태 준비(S3)가 시스템·source·사용자 홈을 손상시키지 않는다.
