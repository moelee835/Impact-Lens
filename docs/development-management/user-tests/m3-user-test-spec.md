...
- **과업별 코드 패턴 적합성:** T4-DI(컨테이너 전용 연결), T5(lambda 내부 호출), T6(getter/record
  accessor)는 참여자 프로젝트에 해당 패턴이 있어야 수행된다. 참여자의 실제 프로젝트에 그 패턴이 없으면
  **해당 sub-scenario만** 진행자 제공 최소 보조 fixture에서 수행한다 — 이때 그것이 합성 보조임을
  기록하고, 그 과업의 관측을 "실제 코드베이스" 결과와 구분해 집계한다(T1~T3의 환경/readiness 관측은
...
- (T4) "이 결과에 대해 어떻게 생각했습니까? 이 도구가 못 보는 호출이 있다면 어떤 것일 것 같습니까?"
  *(개방형 — "완전한가"로 묻지 않는다. §6 금지 단어 "완전"을 사후 질문에서도 진행자가 먼저 꺼내지
  않는다.)*
...
- [ ] T6(callable 오탐)가 존재하고, Java에서 getter/record accessor로 재며(operator/subscript는 Swift/
  ...
  패턴이 없을 때 보조 fixture fallback과 그 집계 분리가 있는가.
