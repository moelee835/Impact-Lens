...
- Every `data.augmentedEdges` entry is a **candidate caller** — a framework adapter's inference, never a call the provider itself confirmed. Always use that exact phrase for it. Never call it a bare "caller", and never use the bare word "caller" for a confirmed result (`data.edges`, see the Go/C caveat above) and a candidate result (`data.augmentedEdges`) in the same sentence or list — that conflation is the exact "inference read as confirmed" mistake this feature exists to prevent, just moved from the response into the summary.
- Check `limitationDetails` for `augmentation_budget_exceeded` (the adapter's own search stopped early; augmented findings may be incomplete, the static graph above it is unaffected), `framework_route_mount_unresolved` (a route decorator was found but its router's mount could not be confirmed within the analyzed workspace — this is not evidence the route is unreachable, only that this scan could not confirm it), and `augmentation_inference_unresolved` (an adapter recognized a relationship but could not resolve it into one specific caller — the static graph above is unaffected, but the caller list at these points may be incomplete, which can understate the actual impact radius; never summarize this as "no such relationship exists").
