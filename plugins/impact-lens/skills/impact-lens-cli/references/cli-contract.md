...
```json
{"schemaVersion":1,"operation":"impact.analyze","ok":true,"runtime":{"cli":{"name":"@impact-lens/cli","version":"0.9.0"},"node":{"version":"22.0.0","major":22,"executable":"node"},"runner":{"source":"release-fallback"}},"data":{},"capabilities":{},"limitations":[],"timings":{}}
```

Failure is one compact JSON document on stderr with a non-zero exit status:

```json
{"schemaVersion":1,"operation":"impact.analyze","ok":false,"runtime":{"cli":{"name":"@impact-lens/cli","version":"0.9.0"},"node":{"version":"22.0.0","major":22,"executable":"node"},"runner":{"source":"global"}},"error":{"code":"provider_initialize_failed","message":"...","retryable":true}}
```
...
- `provider_null_incoming_calls` means the provider answered this specific query with JSON-RPC `null`
  ...
  only through a mechanism a static Call Hierarchy provider cannot see, measured directly against real
  FastAPI code, not assumed: a route handler (`@app.get()` and similar) or a `Depends()` dependency. Both
  are genuinely called by FastAPI at request time - confirmed for `Depends()` by instrumenting the real
  dependency function and hitting the route through `TestClient`, which executes it - just never through a
  call expression anywhere in the analyzed code; the call happens inside FastAPI's own router dispatch or
  dependency resolver, both outside what this preset analyzes. Both converge on this same `null` signal.
  When this code is present, do not state or imply that nothing calls the symbol.
...
- As `error.code` on `ok: false`, with `details.stage: "indexing"`, exit 5, `retryable: true`:
  ...
  ```json
  {
    ...
    "ok": false,
    "runtime": {"cli": {"name": "@impact-lens/cli", "version": "0.9.0"}, "node": {"version": "22.0.0", "major": 22, "executable": "node"}, "runner": {"source": "checkout"}},
    "error": {
