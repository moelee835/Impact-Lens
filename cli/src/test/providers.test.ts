...
// Rule for every test below that overrides `platform` in a `lookup`/`findExecutable()` option (see
// `syntheticPosixDirectory` just below for why): a path handed to that call must be a synthetic POSIX
// string, built by hand, never a value that came from `os.tmpdir()`, `path.join()`, or any other
// host-native path API - not even indirectly through a helper's return value. If the host is Windows,
// a drive-letter colon or a `path.join()`-normalized backslash will falsify the simulation regardless of
// what `platform` claims. This rule has been violated three times in this file (`docs/work/task-fix-cli-test-windows-compat.md`
// has the detail on each), and none of the three were visible until Windows CI actually ran `cli:test`
// for the first time - a review or a type system cannot catch this, only running it on the host it
// pretends to be can.
