...
Impact Lens는 다음 두 구성 요소를 독립적으로 배포합니다. 필요한 것만 설치하거나 둘 다 설치할 수 있습니다.
...
최신 배포 파일은 [GitHub Releases](https://github.com/moelee835/Impact-Lens/releases/latest)에서 받습니다. 아래 예시는 현재 안정 버전인 v0.4.0을 기준으로 합니다.
...
v0.4.0 VSIX를 [직접 다운로드](https://github.com/moelee835/Impact-Lens/releases/download/v0.4.0/impact-lens-0.4.0.vsix)하거나 Release의 `impact-lens-0.4.0.vsix` asset을 받습니다.
...
3. 다운로드한 `impact-lens-0.4.0.vsix`를 선택합니다.
...
목록에 `local.impact-lens@0.4.0`이 있어야 합니다. VS Code UI에서는 Extensions의 Impact Lens 상세 화면에서 버전을 확인합니다.
...
```sh
npm install --global https://github.com/moelee835/Impact-Lens/releases/download/v0.4.0/impact-lens-cli-0.4.0.tgz
```

### tarball을 받은 뒤 전역 설치

CLI tarball을 [직접 다운로드](https://github.com/moelee835/Impact-Lens/releases/download/v0.4.0/impact-lens-cli-0.4.0.tgz)하거나 다음 명령으로 받습니다.

```sh
curl --location --remote-name \
  https://github.com/moelee835/Impact-Lens/releases/download/v0.4.0/impact-lens-cli-0.4.0.tgz
npm install --global ./impact-lens-cli-0.4.0.tgz
```
...
```sh
npm exec --yes \
  --package=https://github.com/moelee835/Impact-Lens/releases/download/v0.4.0/impact-lens-cli-0.4.0.tgz \
  -- impact-lens analyze \
...
v0.4.0 공식 artifact의 SHA-256은 다음과 같습니다.

```text
impact-lens-0.4.0.vsix
1e3214f7bc9747efc3bdbd9ab32101840a17bd9410080cd11dc618331cb6851e

impact-lens-cli-0.4.0.tgz
34e8e945e55ca935b180fcd9f9a37311e87fb10a9cb9b54283dc57bb37cf5e39
```

macOS:

```sh
shasum -a 256 impact-lens-0.4.0.vsix impact-lens-cli-0.4.0.tgz
```

Linux:

```sh
sha256sum impact-lens-0.4.0.vsix impact-lens-cli-0.4.0.tgz
```

Windows PowerShell:

```powershell
Get-FileHash .\impact-lens-0.4.0.vsix -Algorithm SHA256
Get-FileHash .\impact-lens-cli-0.4.0.tgz -Algorithm SHA256
```

계산 결과가 위 값 또는 GitHub Release asset의 digest와 다르면 설치하지 말고 파일을 다시 다운로드합니다.
